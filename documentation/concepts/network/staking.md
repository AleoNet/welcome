...
* **amount_in_microcredits** – minimum = 1 000 000 ( = 1 AC). To enter the committee you need ≥ 10 000 000 000 000 ( = 10 M AC) combined self-bond + delegation.
...
4. If the total delegation is ≥ 10 M AC and the validator was not in `committee` yet, they are added and `metadata[committee_size]` is incremented. (Subjected to network maximum committee size that can be increased with protocol upgrades)
...
* **Self-bond** – validator's own stake (at least 100 AC is required).
