---
id: overview
title: Overview
sidebar_label: Overview
---

The core `sdk/` library offers a wide variety of features to help streamline the development process on Aleo, including account management, program deployment, function execution, Aleo Credit transfers, program state management, and network queries.

## Table of Contents

* [Getting Started](./01_getting_started.md)
* [Creating Aleo Accounts](./02_create_account.md)
* [Deploying Programs](./03_deploy_programs.md)
* [Executing Aleo Programs](./04_execute_programs.md)
* [Transferring Aleo Credits](./05_transfer_credits.md)
* [Managing State](./06_managing_state.md)
    * [Public State: Mappings](./06_managing_state.md#public-state-mappings)
    * [Private State: Records](./06_managing_state.md#private-state-records)
* [Querying the Network](./07_querying_network.md)
    * [Blocks](./07_querying_network.md#blocks)
    * [Transactions and Transitions](./07_querying_network.md#transactions-and-transitions)
    * [Programs](./07_querying_network.md#programs)


## Further Documentation

API documentation for the SDK can be found in the [API Specification](../../category/api-specification) section.

Additi