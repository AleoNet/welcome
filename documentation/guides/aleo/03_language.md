...
Aleo provides several built-in group generators that can be used in programs:

| Name | Description |
|------|-------------|
| `group::GEN` | The generator of the prime order subgroup for the elliptic curve. |
| `aleo::GENERATOR` | A single group point (not indexable). The point `G` used for Aleo account derivation, where `view_key * G = address`. |
| `aleo::GENERATOR_POWERS` | An array of group bases for the Aleo signature and encryption schemes. Use `aleo::GENERATOR_POWERS[Nu32]` to index. |

**`group::GEN`** is the generator of the prime order subgroup for the elliptic curve. It is used for general-purpose elliptic curve operations.

**`aleo::GENERATOR`** is the point `G` satisfies `view_key * G = address` and is used in `g_scalar_multiply` for Aleo account and signature derivation. It is the first element of `aleo::GENERATOR_POWERS`.

**`aleo::GENERATOR_POWERS`** is the group bases for the Aleo signature and encryption schemes, generated by calling `hash_to_curve` on the domain string `"AleoAccountEncryptionAndSignatureScheme0"`. This is used specifically for Aleo accounts and signatures. The array length is 253 and valid indices are `0u32` through `252u32`.

```aleo
function example:
    input r0 as scalar.public;
    input r1 as address.public;
    // Using the elliptic curve generator
    add group::GEN group::GEN into r2;
    
    // Using the Aleo account generator to derive an address from a view key
    mul aleo::GENERATOR r0 into r3;
    
    // Accessing the first element of GENERATOR_POWERS (same as aleo::GENERATOR)
